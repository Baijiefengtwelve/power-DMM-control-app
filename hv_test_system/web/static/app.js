...
function syncNamedSelect(selectId, options, currentValue){
  const el = $(selectId);
  if (!el || !Array.isArray(options) || !options.length) return;
  const prev = el.value;
  const wanted = (currentValue ?? prev ?? "自动判断");
  const same = options.length === el.options.length && options.every((v, i)=> String(el.options[i].value) === String(v));
  if (!same){
    el.innerHTML = "";
  ...
  setValueIfNotDirty(selectId, wanted);
}
...
async function refreshPorts(){
  ...
  const hvPorts = ports?.data?.hv_ports || ports?.data?.data?.hv_ports || [];
  const meterDevices = ports?.data?.meter_devices || ports?.data?.data?.meter_devices || [];
  const hvSel = $("hvPort");
  hvSel.innerHTML = "";
  (hvPorts || []).forEach(p=>{
  ...
  meterTypes.forEach(mt=>{
    ...
    (meterDevices || []).forEach(device=>{
      ...
      opt.value = value;
      opt.textContent = String(device.label || value);
      if (device.tooltip) opt.title = String(device.tooltip);
      sel.appendChild(opt);
    });
...
function applyTelemetry(payload){
  ...
  const powerSourceNames = state?.data?.power_source_names || state?.power_source_names || state?.data?.ui?.power_source_names || state?.ui?.power_source_names || ["自动判断"];
  ...
  if (tp){
    ...
    syncNamedSelect("testPowerSource", powerSourceNames, tp.power_source_name || tp.power_source || "自动判断");
  }
  ...
  if (sp){
    ...
    syncNamedSelect("stabPowerSource", powerSourceNames, sp.power_source_name || sp.power_source || "自动判断");
...
function bindActions(){
  ...
  const dirtyIds = [
    "testStartV","testTargetV","testStepV","testStepDelay","testCycleTime","testPowerSource",
    "stabTargetI","stabRangeI","stabStartV","stabPowerSource","stabSource","stabFreq","stabMaxAdjV","stabAlgo",
    "recordPath"
  ...
  $("btnSaveTestParams").onclick = async ()=>{
    const body = {
      ...
      power_source_name: String(($("testPowerSource") && $("testPowerSource").value) || "自动判断"),
    };
    const r = await postJSON("/api/params/test", body);
    if (r && r.ok){ clearDirty(["testStartV","testTargetV","testStepV","testStepDelay","testCycleTime","testPowerSource"]); }
  };
  ...
  if (btnSaveStab){
    btnSaveStab.onclick = async ()=>{
      const body = {
        ...
        power_source_name: String(($("stabPowerSource") && $("stabPowerSource").value) || "自动判断"),
      ...
      if (r && r.ok){ clearDirty(["stabTargetI","stabRangeI","stabStartV","stabPowerSource","stabSource","stabFreq","stabMaxAdjV","stabAlgo"]); }
    };
  }
