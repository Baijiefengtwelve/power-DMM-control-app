...
from .common import *
from .device_io.workers import SerialIOWorker
...
class HAPS06Controller:
    ...
    power_source_key = "haps06"
    power_source_name = "HAPS06"
    supports_internal_current_readback = False
    ...
    def __init__(self):
        ...
        self.set_voltage_value = 0.0       # 设定电压（读回）
    ...
    # 与 Keithley 控制器对齐的兼容接口
    def set_voltage(self, voltage):
        return self.set_voltage_only(float(voltage))

    def read_voltage(self):
        v = self.read_actual_voltage()
        if v is None:
            v = self.read_set_voltage()
        return v

    def read_current(self):
        # HAPS06 当前工程未实现电源自身电流回读；稳流请改用外部万用表电流。
        return None

    def enable_high_voltage(self):
        if self.is_remote_control:
            return True, "HAPS06远控已启用"
        return self.enable_remote_control()
