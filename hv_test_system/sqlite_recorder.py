...
class SQLiteRecorder:
    ...
    def stop(self, *, timeout_s: float = 5.0):
        ...
        if self._thread:
            ...
            if self._thread.is_alive():
                ...
                self.last_stop_message = f"sqlite recorder stop timed out after {float(timeout_s):.1f}s"
    ...
    def status(self) -> Dict[str, Any]:
        return {
            ...
            "schema_version": int(self.CURRENT_SCHEMA_VERSION),
            "last_stop_ok": self.last_stop_ok,
            "last_stop_message": str(self.last_stop_message or ""),
        }
    ...
    @classmethod
    def _migrate_schema(cls, conn: sqlite3.Connection):
        try:
            current_version = cls._get_schema_version(conn)
            if current_version < 1:
                cls._migrate_to_v1(conn)
            cls._set_schema_version(conn, cls.CURRENT_SCHEMA_VERSION)
            conn.commit()
        except Exception:
            pass
    ...
    def _init_schema(conn: sqlite3.Connection):
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS data (
                ...
                time_text TEXT,
                test_power_name TEXT,
                stabilization_power_name TEXT,
                hv_voltage REAL,
                cathode REAL,
                cathode_unit TEXT,
                gate REAL,
    ...
    def _migrate_to_v1(conn: sqlite3.Connection):
        ...
        for column_name, column_type in wanted_columns.items():
            if column_name not in columns:
                conn.execute(f"ALTER TABLE data ADD COLUMN {column_name} {column_type};")
    ...
    def _flush_batch(self, batch_items) -> bool:
        ...
        try:
            ...
            for it in batch_items:
                ...
                rows.append(
                    (
                        ...
                        int(it.get("ts_ms")),
                        row.get("time_text"),
                        row.get("test_power_name"),
                        row.get("stabilization_power_name"),
                        row.get("hv_voltage"),
                        row.get("cathode"),
                        row.get("cathode_unit"),
                        row.get("gate"),
                        row.get("gate_unit"),
                        row.get("anode"),
                        row.get("anode_unit"),
            ...
            self._conn.executemany(
                """
                INSERT INTO data(
                    run_id, ts_ms, time_text, test_power_name, stabilization_power_name, hv_voltage,
                    cathode, cathode_unit, gate, gate_unit, anode, anode_unit, backup, backup_unit,
                    vacuum, keithley_voltage, gate_plus_anode, gate_plus_anode_unit, anode_cathode_ratio
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                rows,
            )
